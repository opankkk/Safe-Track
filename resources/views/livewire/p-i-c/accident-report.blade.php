...
@section('page-title')
  <div class="d-flex flex-column">
    <span class="font-weight-bold" style="font-size: 1.6rem;">Laporan Kecelakaan (Accident)</span>
  </div>
@endsection
  ...
  <div class="card" style="border-radius:.75rem;">
    ...
    <div class="card-body p-0">
      <div class="table-responsive">
        <table class="table table-striped projects mb-0" id="accidentTable" style="table-layout: fixed; min-width: 1630px;">
          ...
          <tbody>
            @forelse($this->reports as $report)
              <tr>
                <td class="text-center font-weight-bold" style="vertical-align:middle;">{{ $report->report_number }}</td>
                ...
                <td class="text-center" style="vertical-align:middle;">
                  @if($report->plan)
                    <a href="{{ asset('storage/' . $report->plan->file_path) }}" target="_blank" class="btn btn-outline-primary btn-sm btn-round-sm" title="Lihat Bukti Tindak Lanjut">
                ...
                <td class="text-center" style="vertical-align:middle;">
                  @if($report->action)
                    <a href="{{ asset('storage/' . $report->action->file_path) }}" target="_blank" class="btn btn-outline-success btn-sm btn-round-sm" title="Lihat Bukti Perbaikan">
        ...
        <div class="modal-body">
          ...
          <div class="form-group mb-0">
            ...
            <div class="custom-file">
              <input type="file" class="custom-file-input @error('planFile') is-invalid @enderror" wire:model="planFile" id="planFileInput" accept=".pdf,.doc,.docx" onchange="document.getElementById('planFileLabel').innerText = this.files[0] ? this.files[0].name : 'Pilih dokumen...'">
              <label class="custom-file-label" for="planFileInput" id="planFileLabel">
      ...
      <form wire:submit.prevent="submitUploadResult">
        <div class="modal-body">
          ...
          <div class="form-group mb-0">
            <label>Upload Dokumen <span class="text-danger">*</span></label>
            <div class="custom-file">
              <input type="file" class="custom-file-input @error('resultFile') is-invalid @enderror" wire:model="resultFile" id="resultFileInput" accept=".pdf,.doc,.docx" onchange="document.getElementById('resultFileLabel').innerText = this.files[0] ? this.files[0].name : 'Pilih dokumen...'">
              <label class="custom-file-label" for="resultFileInput" id="resultFileLabel">
